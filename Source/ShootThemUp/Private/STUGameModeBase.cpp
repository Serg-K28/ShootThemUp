...
void ASTUGameModeBase::StartPlay() //Коли гра починається викликається дана функція
{
    Super::StartPlay();
    SpawnBots();
    CreateTeamsInfo();
...
void ASTUGameModeBase::ResetOnePlayer(AController* Controller)
{
    ...
    {
        Controller->GetPawn()->Reset(); //Знищуємо нашого героя
    }
    RestartPlayer(Controller);  //Створюємо заново героя
...
bool ASTUGameModeBase::SetPause(APlayerController* PC, FCanUnpause CanUnpauseDelegate)
{
    ...
    if (PauseSet)   // Інколи пауза не прожимається тому робимо перевірку для нашого стана
    {
        StopAllFire();
